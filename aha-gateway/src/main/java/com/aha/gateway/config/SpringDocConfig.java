...
@ConditionalOnProperty(value = "springdoc.api-docs.enabled", matchIfMissing =
        true)
...
class SwaggerDocRegister extends Subscriber<InstancesChangeEvent> {
    private final static String[] EXCLUDE_ROUTES = new String[]{"aha-gateway"
            , "aha-auth", "aha-file", "aha-monitor"};
    @Autowired
    private SwaggerUiConfigProperties swaggerUiConfigProperties;
    @Autowired
    private DiscoveryClient discoveryClient;
    ...
    public void onEvent(InstancesChangeEvent event) {
        Set<AbstractSwaggerUiConfigProperties.SwaggerUrl> swaggerUrlSet =
                discoveryClient.getServices().stream().flatMap(serviceId -> discoveryClient.getInstances(serviceId).stream()).filter(instance -> !StringUtils.equalsAnyIgnoreCase(instance.getServiceId(), EXCLUDE_ROUTES)).map(instance -> {
            AbstractSwaggerUiConfigProperties.SwaggerUrl swaggerUrl =
                    new AbstractSwaggerUiConfigProperties.SwaggerUrl();
            swaggerUrl.setName(instance.getServiceId());
            swaggerUrl.setUrl(String.format("/%s/v3/api-docs",
